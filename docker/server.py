...
BLOCKED_OP_PREFIXES = [
    ...
    "<edit", "<configure", "<import", "<clone", "<commit",
]
...
async def commit_config(description: str = "") -> str:
    ...
    try:
        ...
        if job_el is None or not job_el.text:
            msg_el = root.find(".//msg") or root.find(".//line")
            msg = msg_el.text if msg_el is not None and msg_el.text else "no job returned"
...
async def discard_changes() -> str:
    ...
    try:
        ...
        result = root.find(".//result") or root.find(".//msg") or root
